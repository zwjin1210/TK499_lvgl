...
{    
	Ux_rxdata.Flag = 0;               //从新开始接收新的数据包
	send2_str(cmd);

	if(( reply1 == 0) && (reply2 == 0))                      //不需要接收数据
		return 1;
	
	Delay_nms(waittime);                 //延时
	
	Ux_rxdata.Data_rx_buf[ Ux_rxdata.Data_count]  = '\0';
	printf(" %s\n",Ux_rxdata.Data_rx_buf);
  
	if ((reply1 != 0) && ( reply2 != 0))
		return ((u8)strstr(Ux_rxdata.Data_rx_buf,reply1) || (u8)strstr(Ux_rxdata.Data_rx_buf,reply2)); 
 	
	else if(reply1 != 0)
		return ((u8)strstr(Ux_rxdata.Data_rx_buf, reply1));
	
	else
		return ((u8)strstr(Ux_rxdata.Data_rx_buf,reply2));
}
...
uint8_t ESP8266_Get_LinkStatus ( void )
{
	...
	{
		if ( strstr ( Ux_rxdata.Data_rx_buf, "STATUS:2\r\n" ) )
			return 2;
		
		else if ( strstr ( Ux_rxdata.Data_rx_buf , "STATUS:3\r\n" ) )
			return 3;
		
		else if ( strstr ( Ux_rxdata.Data_rx_buf , "STATUS:4\r\n" ) )
			return 4;	
	}
	return 0;
}
...
uint8_t ESP8266_Get_IdLinkStatus ( void )
{
	...
	if ( ESP8266_Cmd ( "AT+CIPSTATUS", "OK", 0, 500 ) )
	{
		if ( strstr ( Ux_rxdata.Data_rx_buf, "+CIPSTATUS:0," ) )
			ucIdLinkStatus |= 0x01;
		else 
			ucIdLinkStatus &= ~ 0x01;
		
		if ( strstr ( Ux_rxdata.Data_rx_buf, "+CIPSTATUS:1," ) )
			ucIdLinkStatus |= 0x02;
		else 
			ucIdLinkStatus &= ~ 0x02;
		
		if ( strstr ( Ux_rxdata.Data_rx_buf, "+CIPSTATUS:2," ) )
			ucIdLinkStatus |= 0x04;
		else 
			ucIdLinkStatus &= ~ 0x04;
		
		if ( strstr ( Ux_rxdata.Data_rx_buf, "+CIPSTATUS:3," ) )
			ucIdLinkStatus |= 0x08;
		else 
			ucIdLinkStatus &= ~ 0x08;
		
		if ( strstr ( Ux_rxdata.Data_rx_buf, "+CIPSTATUS:4," ) )
			ucIdLinkStatus |= 0x10;
		else 
			ucIdLinkStatus &= ~ 0x10;	
...
u8 ESP8266_Inquire_ApIp ( char * pApIp, uint8_t ucArrayLength )
{
  ...
  ESP8266_Cmd ( "AT+CIFSR", "OK", 0, 500 );
	...
	pCh = strstr ( Ux_rxdata.Data_rx_buf, "APIP,\"" );
...
char * ESP8266_ReceiveString ( FunctionalState enumEnUnvarnishTx )
{
	...
	while ( !Ux_rxdata.Flag);
	Ux_rxdata.Data_rx_buf[Ux_rxdata.Data_count] = '\0';
	
	if ( enumEnUnvarnishTx )
		pRecStr = Ux_rxdata.Data_rx_buf;
	
	else 
	{
		if ( strstr ( Ux_rxdata.Data_rx_buf, "+IPD" ) )
			pRecStr = Ux_rxdata.Data_rx_buf;
	}
	return pRecStr;
}
...
uint8_t ESP8266_CWLIF ( char * pStaIp )
{
  ...
  ESP8266_Cmd ( "AT+CWLIF", "OK", 0, 100 );
	...
	pCh = strstr ( Ux_rxdata.Data_rx_buf, "," );
	
	if ( pCh )
	{
		pCh1 = strstr ( Ux_rxdata.Data_rx_buf, "AT+CWLIF\r\r\n" ) + 11;
	  ucLen = pCh - pCh1;
	}
